...
public:
	Alpha() {
		if (omega == nullptr) {
			omega = new float(1024.0f);
		}
	}
	Alpha(const Alpha& clone) {
		omega = new float();
		*omega = *clone.omega;
	}
	~Alpha() {
...
#pragma region 얕은 복사
	...
	int* beta = alpha;
	*alpha = 10;
	cout << "alpha 변수의 값 : " << alpha << endl;
	cout << "beta 변수의 값 : " << beta << endl;
	cout << "alpha가 가리키는 값 : " << *alpha << endl;
	cout << "beta가 가리키는 값 : " << *beta << endl;
	delete alpha;*/
